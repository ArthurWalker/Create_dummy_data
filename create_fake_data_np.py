...
def get_random_data(i):
    ...
    new_row = [sex,tinhKS,huyenKS,xaKS,soCMTND,ho,dem,ten,ngay,thang,nam]
    return new_row
...
def main():
    ...
    while i<1000000:
        ...
        results = pool.map(get_random_data,[j for j in range(10000)])
        i+=10000
        pool.close()
        df_result = pd.DataFrame(results,columns=['GIOI_TINH','MA_TINH_KS','MA_HUYEN_KS','MA_XA_KS','SO_CMTND','HO','DEM','TEN','NGAY','THANG','NAM'])
        if not os.path.isfile('./new_record3.csv'):
            df_result.to_csv('./new_record3.csv')
        else:
            df_result.to_csv('./new_record3.csv',mode='a',header=False,index_label=False)
        # print ('Join data')
